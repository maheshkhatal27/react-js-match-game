...
class App extends Component {
  ...
  runningTime = () => {
    ...
    if (timeLimit === 0) {
      this.clearIntervalId()
      this.setState(prevState => ({isMatchOn: !prevState.isMatchOn}))
      this.setState({timeLimit: 60})
    } else {
  ...
  playAgainClick = () => {
    this.setState(prevState => ({isMatchOn: !prevState.isMatchOn}))
    this.setState({topScore: 0, timeLimit: 60, currentImage: imagesList[0]})
    this.componentDidMount()
  }

  currentImageTabClick = category => {
    ...
    if (id === category) {
      ...
      // below- setting randomly current display image to be matched

      this.setState({
        currentImage: imagesList[Math.floor(Math.random() * imagesList.length)],
      })
    } else {
      this.setState(prevState => ({isMatchOn: !prevState.isMatchOn}))
  ...
  getImagesOfCurrentTab = () => {
    const {activeTabId} = this.state
    const filteredImagesOfCurrentTab = imagesList.filter(
      eachItem => eachItem.category === activeTabId,
    )
    return filteredImagesOfCurrentTab
  }

  displayImagePanel = () => {
    const {activeTabId, currentImage} = this.state
    const {imageUrl} = currentImage
    const currentTabImages = this.getImagesOfCurrentTab()

    return (
      <div className="main-image-panel">
        <img src={imageUrl} className="to-be-matched-img" alt="match" />
        ...
        <ul className="image-list-container">
          {currentTabImages.map(eachImages => (
            <ImageElement
              key={eachImages.id}
              imageItemDetails={eachImages}
              currentImageTabClick={this.currentImageTabClick}
            />
          ))}
        </ul>
      </div>
  ...
  displayGameStatus = () => {
    ...
    return (
      <WinOrLose currentScore={topScore} playAgainClick={this.playAgainClick} />
    )
  }

  render() {
    const {isMatchOn, topScore, timeLimit} = this.state
    return (
      <div className="match-game-container">
        ...
        <div className="display-images-container">
          {isMatchOn ? this.displayImagePanel() : this.displayGameStatus()}
        </div>
      </div>
    )
  }
